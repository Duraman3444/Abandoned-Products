...
class SampleDataTests(TestCase):
    ...
    def test_sample_data_uniqueness(self):
        ...
        self.assertGreaterEqual(
            len(unique_applicant_emails), 0
        )  # Some applicants may not have emails
    ...
    def test_sample_data_relationships(self):
        ...
        self.assertGreater(
            applicants_with_levels, 0
        )  # At least some applicants should have levels

        # Test that some applicants have documents
        applicants_with_docs = (
            ApplicantDocument.objects.values("applicant").distinct().count()
        )
