...
from admissions.models import (
    Applicant, ApplicantDocument, FeederSchool, AdmissionLevel, 
    AdmissionCheck, ApplicationDecision, ContactLog, OpenHouse
)
...
class SampleDataIntegrityTest(TestCase):
    ...
    def test_student_count_and_uniqueness(self):
        ...
        self.assertGreaterEqual(student_count, 16, f"Expected 16+ students, got {student_count}")
        
        # Verify no duplicate student IDs (UUIDs should be unique by default)
        student_ids = list(students.values_list('id', flat=True))
        self.assertEqual(len(student_ids), len(set(student_ids)), "Student IDs should be unique")
        ...
        for student in students[:5]:  # Check first 5 students
            ...
            self.assertIsNotNone(student.date_of_birth, "Student should have date_of_birth")
    ...
    def test_applicant_count_and_data_quality(self):
        ...
        self.assertGreaterEqual(applicant_count, 10, f"Expected 10+ applicants, got {applicant_count}")
        
        # Verify no duplicate applicant IDs
        applicant_ids = list(applicants.values_list('id', flat=True))
        self.assertEqual(len(applicant_ids), len(set(applicant_ids)), "Applicant IDs should be unique")
        
        # Verify applicants have required fields
        for applicant in applicants[:5]:  # Check first 5 applicants
            self.assertIsNotNone(applicant.first_name, "Applicant should have first_name")
            self.assertIsNotNone(applicant.last_name, "Applicant should have last_name")
            self.assertIsNotNone(applicant.date_of_birth, "Applicant should have date_of_birth")
            self.assertIsNotNone(applicant.applying_for_grade, "Applicant should have applying_for_grade")
    ...
    def test_grade_level_count_and_structure(self):
        ...
        self.assertGreaterEqual(grade_count, 13, f"Expected 13+ grade levels, got {grade_count}")
        
        # Verify grade levels are properly ordered
        ordered_grades = grade_levels.order_by('order')
        previous_order = 0
        for grade in ordered_grades:
            self.assertGreater(grade.order, previous_order, "Grade levels should be properly ordered")
        ...
        self.assertEqual(len(grade_names), len(set(grade_names)), "Grade level names should be unique")
    ...
    def test_grade_level_assignments(self):
        ...
        self.assertGreater(students_with_grades.count(), 0, "Students should be assigned to grade levels")
        
        # Verify all assigned grades exist in GradeLevel table
        assigned_grade_ids = set(students_with_grades.values_list('grade_level_id', flat=True))
        existing_grade_ids = set(GradeLevel.objects.values_list('id', flat=True))
        self.assertTrue(assigned_grade_ids.issubset(existing_grade_ids), 
                       "All student grade assignments should reference valid grade levels")
        
        # Test applicant grade assignments  
        applicants_with_grades = Applicant.objects.filter(applying_for_grade__isnull=False)
        self.assertGreater(applicants_with_grades.count(), 0, "Applicants should be assigned to grade levels")
        
        # Verify all applicant grade assignments are valid
        applicant_grade_ids = set(applicants_with_grades.values_list('applying_for_grade_id', flat=True))
        self.assertTrue(applicant_grade_ids.issubset(existing_grade_ids),
    ...
    def test_applicant_document_associations(self):
        ...
        self.assertGreaterEqual(document_count, 20, f"Expected 20+ documents, got {document_count}")
        ...
        self.assertTrue(document_applicant_ids.issubset(existing_applicant_ids),
        ...
        for document in documents[:10]:  # Check first 10 documents
            self.assertIsNotNone(document.applicant, "Document should be associated with an applicant")
            self.assertIsNotNone(document.check, "Document should be associated with an admission check")
            self.assertTrue(hasattr(document, 'file'), "Document should have file field")
    ...
    def test_admission_level_structure(self):
        ...
        self.assertGreater(admission_levels.count(), 0, "Should have admission levels created")
        
        # Verify proper ordering
        ordered_levels = admission_levels.order_by('order')
        previous_order = 0
        for level in ordered_levels:
            self.assertGreater(level.order, previous_order, "Admission levels should be properly ordered")
    ...
    def test_admission_checks_associations(self):
        ...
        self.assertGreater(admission_checks.count(), 0, "Should have admission checks created")
        ...
        self.assertTrue(check_level_ids.issubset(existing_level_ids),
    ...
    def test_emergency_contact_associations(self):
        ...
        self.assertGreater(emergency_contacts.count(), 0, "Should have emergency contacts created")
        
        # Check that some students have emergency contacts
        students_with_contacts = Student.objects.filter(emergency_contacts__isnull=False).distinct()
        self.assertGreater(students_with_contacts.count(), 0, 
    ...
    def test_feeder_school_structure(self):
        ...
        self.assertGreater(feeder_schools.count(), 0, "Should have feeder schools created")
    ...
    def test_application_decisions_structure(self):
        ...
        self.assertGreater(decisions.count(), 0, "Should have application decisions created")
        
        # Verify decisions have required fields
        for decision in decisions:
            self.assertIsNotNone(decision.name, "Application decision should have a name")
            self.assertIsInstance(decision.is_positive, bool, "is_positive should be boolean")
    ...
    def test_data_consistency_totals(self):
        ...
        self.assertEqual(Applicant.objects.count(), 10, "Should have exactly 10 applicants") 
        self.assertEqual(GradeLevel.objects.count(), 13, "Should have exactly 13 grade levels")
        self.assertGreaterEqual(ApplicantDocument.objects.count(), 20, "Should have 20+ documents")
        
        # Verify no orphaned records
        self.assertEqual(Student.objects.filter(grade_level__isnull=True).count(), 0,
                        "All students should have grade assignments")
        self.assertEqual(Applicant.objects.filter(applying_for_grade__isnull=True).count(), 0,
    ...
    def test_referential_integrity(self):
        ...
        for student in Student.objects.all()[:10]:  # Sample check
            if student.grade_level:
                self.assertTrue(GradeLevel.objects.filter(id=student.grade_level.id).exists(),
        ...
        for applicant in Applicant.objects.all()[:10]:  # Sample check
            if applicant.applying_for_grade:
                self.assertTrue(GradeLevel.objects.filter(id=applicant.applying_for_grade.id).exists(),
                               f"Applicant {applicant.id} references non-existent grade level")
            if applicant.school_year:
                self.assertTrue(SchoolYear.objects.filter(id=applicant.school_year.id).exists(),
        ...
        for document in ApplicantDocument.objects.all()[:10]:  # Sample check
            self.assertTrue(Applicant.objects.filter(id=document.applicant.id).exists(),
                           f"Document {document.id} references non-existent applicant")
            if hasattr(document.check, 'id'):
                self.assertTrue(AdmissionCheck.objects.filter(id=document.check.id).exists(),
