...
from academics.models import Course, Enrollment, Assignment, Grade
...
def global_search_view(request):
    ...
    if query:
        ...
        results['total_count'] = (
            len(results['students']) + len(results['courses']) + 
            len(results['teachers']) + len(results['assignments'])
        )
        ...
        SearchHistory.objects.filter(
            user=request.user,
            query=query
        ).update(results_count=results['total_count'])
    
    # Get filter options
    grade_levels = Student.objects.values_list('grade_level__name', flat=True).distinct()
...
def search_teachers(query, filters):
    ...
    return [{
        ...
        'courses_count': teacher.taught_sections.count() if hasattr(teacher, 'taught_sections') else 0,
...
def search_suggestions_api(request):
    ...
    if category in ['general', 'teacher_name']:
        ...
        live_suggestions.extend([
            f"{t.first_name} {t.last_name}" for t in teachers if t.first_name and t.last_name
        ])
