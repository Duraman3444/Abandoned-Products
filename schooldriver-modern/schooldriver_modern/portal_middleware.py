...
class PortalAccessMiddleware:
    ...
    def process_request(self, request):
        ...
        if request.path.startswith("/student/"):
            logger.info(
                f"🏫 STUDENT PORTAL ACCESS: {request.user.username} -> {request.path}"
            )
            ...
            if user_role != UserRoles.STUDENT:
                logger.warning(
                    f"❌ DENIED: User {request.user.username} with role {user_role} tried to access {request.path}"
                )
                messages.error(
                    request,
                ...
                logger.info(
                    f"✅ ALLOWED: User {request.user.username} accessing {request.path}"
                )
