...
class UserProfile(models.Model):
    ...
    # Emergency Contacts
    emergency_contact_1_name = models.CharField(
        max_length=100, blank=True, verbose_name="Primary Contact Name"
    )
    emergency_contact_1_relationship = models.CharField(
        max_length=50, blank=True, verbose_name="Primary Contact Relationship"
    )
    emergency_contact_1_phone = models.CharField(
        max_length=20, blank=True, verbose_name="Primary Contact Phone"
    )

    emergency_contact_2_name = models.CharField(
        max_length=100, blank=True, verbose_name="Secondary Contact Name"
    )
    emergency_contact_2_relationship = models.CharField(
        max_length=50, blank=True, verbose_name="Secondary Contact Relationship"
    )
    emergency_contact_2_phone = models.CharField(
        max_length=20, blank=True, verbose_name="Secondary Contact Phone"
    )
...
class SecurityEvent(models.Model):
    ...
    username = models.CharField(
        max_length=150, help_text="Username attempted (for failed logins)"
    )
    ...
    def __str__(self):
        ...
        return (
            f"{self.get_event_type_display()} - {user_identifier} at {self.timestamp}"
        )

    @classmethod
    def log_event(
        cls, event_type, user=None, username=None, request=None, **extra_details
    ):
