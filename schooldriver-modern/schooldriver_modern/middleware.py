...
class LoginAttemptLimitingMiddleware:
    ...
    def get_failed_attempts(self, identifier):
        ...
        cache.set(
            cache_key, attempts_data, timeout=int(self.lockout_duration.total_seconds())
        )
...
def handle_failed_login(sender, credentials, request, **kwargs):
    ...
    SecurityEvent.log_event(
        "LOGIN_FAILED", username=username, request=request, failed_attempts=failed_count
    )
...
def handle_successful_login(sender, user, request, **kwargs):
    ...
    SecurityEvent.log_event("LOGIN_SUCCESS", user=user, request=request)
...
def check_login_allowed(username):
    ...
    if failed_count > 0:
        return (
            True,
            f"Warning: {failed_count} failed login attempts. {remaining} attempts remaining.",
        )
