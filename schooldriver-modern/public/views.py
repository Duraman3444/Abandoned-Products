...
def home_view(request):
    ...
    try:
        ...
        upcoming_events = OpenHouse.objects.filter(
            date__gte=timezone.now(), is_active=True
        ).order_by("date")[:3]
        ...
        context = {"error": "Unable to load school statistics at this time."}
...
def admissions_view(request):
    ...
    try:
        ...
        admission_levels = AdmissionLevel.objects.filter(is_active=True).order_by(
            "order"
        )
        ...
        application_deadlines = [
            {
                "deadline": "February 1",
                "description": "Regular Decision Applications Due",
            },
        ...
        "departments": [
            {
                ...
                "phone": "(555) 123-4568",
                "email": "admissions@privateschool.edu",
            },
            {
                "name": "Student Services",
                "phone": "(555) 123-4569",
                "email": "students@privateschool.edu",
            },
            {
                "name": "Parent Relations",
                "phone": "(555) 123-4570",
                "email": "parents@privateschool.edu",
            },
        ],
    }
...
def programs_view(request):
    ...
    try:
        ...
        elementary_count = Student.objects.filter(
            grade_level__in=elementary_grades, is_active=True
        ).count()

        middle_count = Student.objects.filter(
            grade_level__in=middle_grades, is_active=True
        ).count()

        high_school_count = Student.objects.filter(
            grade_level__in=high_school_grades, is_active=True
        ).count()
