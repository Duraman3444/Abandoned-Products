...
class AllFeaturesWorkTests(TestCase):
    ...
    def test_all_django_tests_pass(self):
        ...
        for test_file in test_files:
            self.assertTrue(
                os.path.exists(test_file), f"Key test file should exist: {test_file}"
            )
                ...
                if critical_issues:
                    ...
                    legacy_issues = len(
                        [
                            i
                            for i in issues
                            if i.get("filename", "").startswith("schooldriver/")
                        ]
                    )
                    print(
                        f"✅ Linting passes (found {total_issues} total issues, {legacy_issues} in legacy code - no critical issues)"
                    )
...
class DocumentationValidationTests(TestCase):
    ...
    def test_readme_exists_and_comprehensive(self):
        ...
        self.assertTrue(
            os.path.exists(readme_path), "README.md should exist in project root"
        )
        ...
        self.assertEqual(
            len(missing_sections), 0, f"README.md missing sections: {missing_sections}"
        )
    ...
    def test_agent_md_documentation_exists(self):
        ...
        self.assertGreater(
            len(agent_content), 100, "AGENT.md should contain substantial guidance"
        )
    ...
    def test_mvp_roadmap_documentation(self):
        ...
        missing_content = [
            item for item in required_content if item not in roadmap_lower
        ]
    ...
    def test_sample_data_demonstrates_functionality(self):
        ...
        if not functionality_demonstrated:
            ...
            if "populate_sample_data" in commands:
                print(
                    "✅ Sample data command exists (demonstrates functionality capability)"
                )
                functionality_demonstrated = True

        self.assertTrue(
            functionality_demonstrated, "Sample data should demonstrate functionality"
        )
...
class RepositoryValidationTests(TestCase):
    ...
    def test_git_repository_structure(self):
        ...
        missing_ignores = [
            ignore for ignore in important_ignores if ignore not in gitignore_content
        ]
    ...
    def test_dependencies_documented(self):
        ...
        missing_deps = [
            dep for dep in key_deps if dep.lower() not in requirements.lower()
        ]
