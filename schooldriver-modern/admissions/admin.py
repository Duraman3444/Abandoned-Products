...
class ApplicantAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            "Contact Information",
            {"fields": ("email", ("street", "city"), ("state", "zip_code"))},
        ),
        ("Family Information", {"fields": ("living_situation",)}),
        (
            "Admission Process",
            {
                "fields": (
        ...
        (
            ...
            {"fields": ("notes", "special_circumstances"), "classes": ("collapse",)},
        ),
        (
    ...
    def get_admission_progress(self, obj):
        ...
        if obj.level:
            ...
            return format_html(
                ...
                color,
                int(percentage),
                obj.level.name if obj.level else "Not Started",
            )
        return "Not Started"
    ...
    def mark_ready_for_enrollment(self, request, queryset):
        ...
        self.message_user(
            request, f"{updated} applicants marked as ready for enrollment."
        )

    mark_ready_for_enrollment.short_description = (
        "Mark selected applicants as ready for enrollment"
    )
    ...
    advance_to_next_level.short_description = (
        "Advance eligible applicants to next level"
    )
...
class ContactLogAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("applicant", ("contact_type", "contacted_by"), "summary")}),
        ("Follow-up", {"fields": ("follow_up_needed", "follow_up_date")}),
        ("System Info", {"fields": ("contact_date",), "classes": ("collapse",)}),
    )
...
class OpenHouseAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("name", ("date", "is_active"), "description", "capacity")}),
        (
            "Attendance",
            {
                "fields": (
...
class ApplicantDocumentAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("File Details", {"fields": (("file_extension", "file_size_mb"), "notes")}),
        (
            "Verification",
            {
                "fields": (
