...
from .serializers import (
    ApplicantSerializer, ApplicantCreateSerializer, ApplicantSummarySerializer,
    FeederSchoolSerializer, AdmissionLevelSerializer, AdmissionCheckSerializer,
    ApplicationTemplateSerializer, ApplicantFileSerializer
)


@extend_schema_view(
    list=extend_schema(description="List all feeder schools", tags=["admissions"]),
    retrieve=extend_schema(description="Get feeder school details", tags=["admissions"]),
    create=extend_schema(description="Create a new feeder school", tags=["admissions"]),
    update=extend_schema(description="Update feeder school information", tags=["admissions"]),
...
@extend_schema_view(
    list=extend_schema(description="List admission process levels", tags=["admissions"]),
    retrieve=extend_schema(description="Get admission level details", tags=["admissions"]),
...
@extend_schema_view(
    list=extend_schema(description="List admission requirements/checks", tags=["admissions"]),
    retrieve=extend_schema(description="Get admission check details", tags=["admissions"]),
...
@extend_schema_view(
    list=extend_schema(description="List all applicants with filtering", tags=["admissions"]),
    retrieve=extend_schema(description="Get detailed applicant information", tags=["admissions"]),
    create=extend_schema(description="Create a new applicant", tags=["admissions"]),
    update=extend_schema(description="Update applicant information", tags=["admissions"]),
...
class ApplicantViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields = [
        'current_level', 'school_year', 'target_grade_level',
        'feeder_school', 'gender'
    ]
    ...
    def can_advance(self, request, pk=None):
        ...
        return Response({
            ...
            'current_level': applicant.current_level.name if applicant.current_level else None,
    ...
    def stats(self, request):
        ...
        stats = {
            ...
            ).order_by('-application_date')[:5].count()
        }
        ...
        grade_counts = queryset.values(
            'target_grade_level__name'
        ).distinct().count()
