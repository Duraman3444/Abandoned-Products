...
class FeederSchool(models.Model):
    ...
    school_type = models.CharField(
        max_length=50, blank=True, help_text="Public, Private, Charter, etc."
    )
...
class AdmissionLevel(models.Model):
    ...
    order = models.PositiveIntegerField(
        unique=True, help_text="Order in admission process, 1 being first"
    )
...
class AdmissionCheck(models.Model):
    ...
    level = models.ForeignKey(
        AdmissionLevel, on_delete=models.CASCADE, related_name="checks"
    )
...
class Applicant(models.Model):
    ...
    current_school = models.ForeignKey(
        FeederSchool, on_delete=models.SET_NULL, null=True, blank=True
    )
    current_school_name = models.CharField(
        max_length=200, blank=True, help_text="If current school not in our feeder list"
    )

    # Family information
    living_situation = models.CharField(
        max_length=20, choices=LIVING_SITUATION_CHOICES, blank=True
    )
    parent_guardians = models.ManyToManyField(
        EmergencyContact, blank=True, help_text="Parent/guardian contacts"
    )
    siblings = models.ManyToManyField(
        Student, blank=True, help_text="Current students who are siblings"
    )
    ...
    completed_checks = models.ManyToManyField(
        AdmissionCheck, blank=True, help_text="Requirements completed by applicant"
    )

    # Application decision
    decision = models.ForeignKey(
        ApplicationDecision, on_delete=models.PROTECT, null=True, blank=True
    )
    decision_date = models.DateField(null=True, blank=True)
    decision_by = models.CharField(
        max_length=200, blank=True, help_text="Who made the decision"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.applicant_id:
            ...
            last_applicant = (
                Applicant.objects.filter(applicant_id__startswith=f"A{year}")
        ...
        primary_parent = (
            self.parent_guardians.first()
        )  # Will need to be called after save
    ...
    def get_next_required_checks(self):
        ...
        return self.level.checks.filter(is_required=True, is_active=True).exclude(
...
class ApplicantDocument(models.Model):
    ...
    uploaded_by = models.CharField(
        max_length=200, blank=True, help_text="Staff member who uploaded"
    )
    notes = models.TextField(blank=True, help_text="Additional notes about document")
    is_verified = models.BooleanField(
        default=False, help_text="Document has been reviewed and approved"
    )
    verified_by = models.CharField(
        max_length=200, blank=True, help_text="Staff member who verified"
    )
...
class ContactLog(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    applicant = models.ForeignKey(
        Applicant, on_delete=models.CASCADE, related_name="contact_logs"
    )
    ...
    contacted_by = models.CharField(
        max_length=200, help_text="Staff member who made contact"
    )
...
class OpenHouse(models.Model):
    ...
    attendees = models.ManyToManyField(
        Applicant, blank=True, help_text="Applicants who attended"
    )
