...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"🎓 Creating demo grades for {num_years} school years for user: {student_username}"
        )

        # Get or create the student
        try:
            user = User.objects.get(username=student_username)
            student = Student.objects.filter(primary_contact_email=user.email).first()
            if not student:
        ...
        for school_year in school_years:
            ...
            for course_name, course_code, credit_hours in course_templates:
                ...
                for cat_name in ["Homework", "Quiz", "Test", "Project"]:
                    category, created = AssignmentCategory.objects.get_or_create(
                        name=cat_name, defaults={"weight": 25.0}
                    )
                ...
                for assign_name, assign_type, max_points in assignment_templates:
                    ...
                    # Create assignment
                    due_date = school_year.start_date + timedelta(
                        days=random.randint(30, 250)
                    )
                    assigned_date = due_date - timedelta(
                        days=random.randint(7, 21)
                    )  # Assigned 1-3 weeks before due
                    ...
                    if created:
                        ...
                        base_performance = random.uniform(
                            75, 95
                        )  # Base student performance
                        grade_percentage = base_performance * year_gpa_variance
                        grade_percentage = min(
                            100, max(60, grade_percentage)
                        ...
                        Grade.objects.create(
                            ...
                            is_late=random.choice(
                                [False, False, False, True]
                            ),  # 25% chance late
                            is_excused=False,
                            comments=f"Good work on {assign_name}!"
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"✅ Created courses and grades for {school_year.name}"
                )
            )
